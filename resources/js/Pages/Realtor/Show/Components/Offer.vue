<template>
  <Box>
    <template #header>
      Offer #{{ offer.id }} 
      <span v-if="offer.accepted_at" class="dark:bg-green-900 dark:text-green-200 bg-green-200 text-green-900 p-1 rounded-md ml-1">ACCEPTED</span>
    </template>
    ...
    <section class="flex items-center justify-between">
      ...
      <div>
        <Link v-if="!isSold && !isDeleted" :href="route('realtor.offer.accept', {offer: offer.id})" class="btn-outline text-xs" as="button" method="put">Accept</Link>
      </div>
    </section>
  </Box>
</template>
